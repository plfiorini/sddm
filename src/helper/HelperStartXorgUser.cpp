...
private:
    ...
    {
        // Client environment
        auto clientEnv = QProcessEnvironment::systemEnvironment();
        clientEnv.insert(QStringLiteral("DISPLAY"), m_display);
        clientEnv.insert(QStringLiteral("XAUTHORITY"), m_xauth.authPath());

        // Start the client process
        qInfo("Running client: %s", qPrintable(m_clientCmd));
        if (!startProcess(m_clientCmd, clientEnv, &m_clientProcess))
            return false;
